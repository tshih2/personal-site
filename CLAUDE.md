...
- **同一個 `afterParagraph` 值如果放兩項以上素材,會自動合併成一個可點擊切換的輪播(`buildContinuousCarousel()`),不是照原本邏輯垂直堆疊——2026-09-06 加的,起因是 OkoEcho 的 OVERVIEW 想放 4 張產品截圖,但單張全尺寸素材(`w-full`,原始長寬比)已經常常撐得很高,4 張直接堆疊會佔掉一整個螢幕高度以上。** 分組依據就是既有的 `afterParagraph` 欄位,資料檔不需要額外欄位標記「這幾張要輪播」——只要給同一個 `afterParagraph` 值就自動成組;只有一項的組合完全比照原本 `buildContinuousMedia()` 的單張渲染,既有資料檔(每項素材各自不同 `afterParagraph`)不受影響。素材尺寸沿用跟單張素材完全一樣的 `buildMediaItem()`(`w-full`,原始長寬比,不裁切、不縮小)——**第一版曾經改成 `max-height` + `object-contain` 塞進固定高度容器,Tim 覺得圖片被縮小了,要求「圖片size改回原本的樣子」但保留點擊切換,所以改成外層 `.continuous-carousel-viewport` 的高度動態量測「目前顯示的那張」自然渲染高度(`initContinuousCarousels()` 的 `measure()`/`setViewportHeight()`),不是寫死的固定值**——每張圖高度可能差很多,切換時 viewport 本身的高度也會跟著用 GSAP tween 過去,不是瞬間跳動;素材桌面版/手機版兩個 `<img>` 用 `hidden`/`block` 依斷點切換顯示,沒顯示的那個是 `display:none` 天生不佔版面高度,所以直接量 slide 自己的 `getBoundingClientRect().height` 就會自動只反映目前斷點實際顯示的版本,不需要另外判斷現在是哪個斷點。等待素材真的載入完成才量測的作法(`img.complete`/`load`、`video.readyState`/`loadedmetadata`)跟 `initMediaColumnHeights()` 對圖片/影片的既有慣例一致。下方有一列可點擊的圓點指示器(顏色沿用既有色票:目前顯示的 `bg-ink`、其餘 `bg-black/15`,跟手風琴版面的圓點指示器同一套配色語言),點擊用 GSAP `autoAlpha` 交叉淡出淡入切換(跟全站其他換圖/開關動畫同一個慣例)。**圖片的點擊行為維持不變、不會被輪播搶掉**——輪播只透過圓點切換「目前顯示哪一張」,點擊圖片本身還是開啟全螢幕 lightbox(`initLightbox()` 完全沒有改,輪播的每個 slide 一樣掛 `media-item`/`data-lightbox-src`,`autoAlpha:0` 的隱藏 slide 因為 `visibility:hidden` 天生就點不到,不需要另外處理 `pointer-events`),lightbox 開著時左右箭頭一樣能在同一組素材之間導覽,包含輪播裡沒在顯示的那幾張。
- **2026-09-03 加了自動播放(每 2 秒換下一張,循環)+ 左右翻頁箭頭按鈕**——Tim 要求輪播「自動每兩秒翻動一次」並且「左右增加一個 button 去翻頁」,不是只能靠下方圓點切換。箭頭按鈕是圓形(`w-9/w-10 rounded-full border-black/15`,沿用手風琴收合按鈕的既有視覺語言,不是另外發明新樣式),背景 `bg-cream/90` 半透明蓋在素材上方,不管底下圖片/影片顏色深淺都維持可辨識;按鈕是 viewport 的直接子元素(跟 `.carousel-slide` 同層,不是塞進 slide 裡面),點擊不會被 `initLightbox()` 誤判成點了圖片本身,兩種互動仍然分開。自動播放用 `window.setInterval`(`initContinuousCarousels()`),滑鼠 hover 進輪播範圍(`mouseenter`)或分頁切到背景/隱藏(`visibilitychange` 的 `document.hidden`)會暫停,離開/切回來才繼續——避免使用者想仔細看某一張或點按鈕時被自動跳圖打斷。手動點圓點/箭頭切換後一律重新從頭計時(不是繼續沿用舊的 2 秒倒數),不然手動選好一張的瞬間可能下一秒就被自動播放蓋掉,體感很奇怪。`prefers-reduced-motion` 時完全不啟動自動播放(圓點/箭頭手動切換還是正常可用),沿用全站對動態效果的既有處理慣例。**用 Playwright 驗證自動播放間隔時,不能直接拿「頁面 `goto()` 完成後過了多久」當基準——`goto()` 預設會等到 `load` 事件(包含所有圖片/影片下載完成)才 resolve,大型素材可能已經讓計時器跑掉一大段,第一次換圖看起來會比 2 秒快很多;正確做法是量「連續兩次換圖之間」的間隔,這個間隔才會穩定落在 2000ms 左右。**
- **段落跟素材都支援 `title`/`align`,2026-09-03 加的**——`content` 陣列每一項可以維持純字串(等同 `{ text: 字串 }`,舊寫法照樣可以用,`normalizeContinuousParagraph()` 統一轉換),也可以寫成 `{ text, title?, align? }` 物件:
  - `title`(可省略)是這一段自己專屬的小標題,顯示在段落文字正上方——這跟這個 section 本身的 `<h2>` 大標題是兩層不同的東西,不要混為一談,但**字體 2026-09-06 改成刻意跟 `<h2>` 共用同一套 `font-geistmono text-xs text-label uppercase`**(`<h3 class="mb-4 font-geistmono text-xs text-label uppercase">`),讓兩層標題視覺上讀作同一種「標籤」角色——原本是獨立的 `font-geist font-semibold text-sm text-ink`(比較像次標題的字重字級),MPAA 這個作品的資料檔用了這個欄位後,Tim 覺得應該跟 section 標題視覺統一,所以改掉。以前的做法是把小標題寫死進 `<strong>...</strong>` 塞進同一段文字裡,現在拆成獨立欄位是因為段落內文本身要套用完全不同的字體(見下面 Arial 那條),繼續塞進同一段字串會讓標題也被迫套用內文的字體/字級。
  - `align`(可省略,預設 `'left'`,另外支援 `'right'`/`'center'`)決定這一段文字在內容欄裡貼齊哪一側或置中,用 `mr-auto`(left)/`ml-auto`(right)/`mx-auto`(center)做——**不是文字自己的 `text-align`**,是整個文字區塊(一般段落 `max-w-3xl`,`style: 'quote'` 段落是 `max-w-xl`,見下面)在較寬的內容欄裡的位置。段落 `align: 'right'` 搭配鄰近素材(反過來靠左),可以做出雜誌式的 Z 字型交錯閱讀動線。`media[]` 的每一項也支援同一個 `align` 欄位(`buildContinuousMedia()`),資料格式還在,但目前不會有視覺效果,見下一條。段落跟素材共用同一個 `continuousAlignClass()` 小工具算對照表,不是兩處各自寫一份 if/else,之後如果要再加新的 align 值只需要改一個地方。
  - **`style: 'quote'`(可省略)套用拉出式引言(pull quote)的樣式**——斜體、字級加大到 `16pt`(一般內文是 `12pt`)、文字本身 `text-align: center`、顏色從 `text-muted` 換成更深的 `text-ink`(引言通常想要比一般內文更搶眼)、區塊寬度也從一般段落的 `max-w-3xl` 縮成 `max-w-xl`(576px,拉出式引言本來就該比正文窄一截,不是佔滿整個內容欄)。`style` 是獨立欄位,不會自動連動 `align`——引言通常會搭配 `align: 'center'` 一起用(讓整個引言區塊也在內容欄裡置中,不是只有文字本身置中卻整塊貼左),但兩者要各自明確指定,不要預期設了 `style: 'quote'` 就自動置中區塊。
...
- **第二欄的章節導覽**(`initContinuousNavigation()`)垂直排列,每個連結對應一個 section 的錨點。點擊會平滑捲動第三欄(不是 `window`)到對應 section,同時用 `history.replaceState()`(不是 `pushState`)更新網址 hash,不會多留一筆瀏覽紀錄。**目前捲到哪個 section,對應連結會自動變 `text-ink`(其餘維持 `text-label`)**,靠 `IntersectionObserver` 做 scrollspy——`rootMargin: '-20% 0px -70% 0px'` 把偵測範圍收窄成螢幕頂端往下一小段,哪個 section 的標題進到這段範圍就算「目前這個」,不需要自己手算捲動位置對應第幾個 section。**2026-09-03 拿掉了 active 狀態的底線**(原本疊加 `border-b border-ink`)——Tim 覺得底線太搶眼,現在 active 只靠顏色深淺區分(`text-ink` vs `text-label`),不要再加回底線。連結本身維持 `inline-block`(不是 `block`)的理由不變:就算之後又想加底線一類的裝飾,底線也該貼齊文字寬度,不要撐滿整欄。
  - **平板/手機(`<lg:`)2026-09-03 改成 hamburger,不再是桌面版那種一路攤開的垂直清單**——Tim 覺得窄螢幕上把整份 section 清單攤開太佔位置。`#continuousNavToggle` 是一顆 `lg:hidden` 的按鈕(桌面版完全不出現),點擊展開/收合 `#continuousNavList`(GSAP `height` 動畫在 `0`/`'auto'` 之間 tween,語意上跟手風琴的展開/收合是同一種模式,但這裡是獨立寫的一小段邏輯,不是重用 `initAccordions()`——那個函式綁死在 case study 的手風琴內容格式上,跟這裡「純粹是章節導覽清單開關」的情境不一樣)。圖示用同一個 `<path>` 元素動態換 `d` 屬性在漢堡(三條線)跟叉叉(X)之間切換,不是切換兩個不同的 `<svg>`。**點擊 section 連結不會自動收合選單**——原本第一版做了「點連結自動關閉」,但 2026-09-03 當天 Tim 明確要求選單要「持續展開著直到被使用者關閉」,所以拿掉了連結上的收合監聽器,只剩點 hamburger 本身(再點一次切換開關)或點選單以外的地方(見下面 `handleDocumentClick`)這兩種使用者主動關閉的方式;連結原本「平滑捲動到對應 section」的點擊行為完全不受影響,是獨立的另一個監聽器。**這段收合行為只在 `(max-width: 1023px)` 這個 `gsap.matchMedia()` 範圍內註冊**——寬度跨過 `lg:` 斷點(1024px)時,GSAP 會自動 revert 掉用 `gsap.set`/`gsap.to` 設過的 inline `height`,不會殘留一個把清單卡在收合狀態的 inline style 蓋掉桌面版原本該是一路展開的樣子(`#continuousNavList` 也額外掛了 `lg:!h-auto lg:!overflow-visible` 當保險)。但 DOM 的 `click` 監聽器不在 GSAP matchMedia 的自動 revert 範圍內,所以 `mm.add()` 的 callback 自己回傳一個 cleanup 函式手動移除監聽器,避免使用者把視窗寬度來回拖過 `lg:` 斷點時,同一個按鈕疊加好幾個重複的監聽器(已用 Playwright 把視窗寬度在桌面/手機之間來回切換三輪,確認每次點擊還是只切換一次開/關狀態,沒有累積出重複觸發)。**這個註解裡如果要引用 Tailwind class 名稱,不能用反引號包起來**——這整個 `<nav>` 區塊是包在 `buildContinuousHtml()` 的樣板字串(用反引號 `` ` `` 界定)裡的 HTML 註解,註解內文本身如果又出現字面上的反引號字元,會被 JS 解析器誤判成樣板字串提前結束,導致整份檔案直接語法錯誤(`Unexpected identifier`)、整個 case study 頁面全部作品都會壞掉——這個坑已經在加這個功能時親自踩過兩次(第一次是加 hamburger 本身的說明,第二次是後續補 sticky/fixed 的說明時又犯了同一個錯),補的方式是拿掉註解裡的反引號、改用純文字或別的標記方式引用 class 名稱。
  - **2026-09-03 同一天,Tim 要求這顆 hamburger 按鈕捲頁時要留在畫面上,一開始用 `position: sticky top-0` 完全沒有效果——實測捲動時整個按鈕跟著頁面一起飄走,行為跟 `position: static`一模一樣,原因是全站唯一的橫向捲動防護(`css/style.css` 的 `html, body { overflow-x: hidden }`,見上面「元件慣例」的「邊界保護」)。** CSS 規格規定:一個元素的 `overflow-x`/`overflow-y` 只要有一軸不是 `visible`,另一軸如果原本是 `visible` 就會被瀏覽器強制改成 `auto`——`html`/`body` 都設了 `overflow-x: hidden`,連帶讓兩者的 `overflow-y` 也被迫變成 `auto`,使 `body` 看起來「有捲動機制」。`position: sticky` 是相對「離自己最近、有捲動機制的祖先」定位,於是把 `body` 誤判成那個祖先——但用 Playwright 量過 `document.scrollingElement` 其實是 `html`,`body.scrollTop` 在整個捲動過程中都停在 0(用滑鼠滾輪真正在捲的是 `html`,不是 `body`),`body` 本身根本沒有真的在捲,套用 sticky 的元素自然只能眼睜睜跟著整個 `body` 被 `html` 捲走。**修法是改用 `position: fixed`**(相對視窗定位,不看任何祖先的 `overflow` 設定,除非祖先有 `transform`,這裡沒有),徹底繞開這整條 `overflow-x`/`overflow-y` 耦合鏈,不需要也不能去動 `html, body` 那條全站防護規則本身(那是刻意集中在一個地方的統一規則,不能因為這裡卡住就在別處加類似規則繞過去)。副作用是 `fixed` 元素會整個脫離文件流,一開始(還沒捲動時)就直接蓋在第一欄最上面,所以第一欄(`intro-col`)另外加了 `pt-20`(`lg:pt-0` 取消)把內容往下推、讓出這個固定頂欄的高度——語意上不是「捲到某個位置才出現」的傳統 sticky,是從頁面一開始載入就固定貼在螢幕最上緣的持續頂欄,兩者體感不完全一樣,但這是繞開前述限制之後唯一可靠的做法。**如果之後在這個網站別的地方也想用 `position: sticky`,一定要先想到這條全站的 `overflow-x: hidden` 規則可能讓它完全失效——不是隨便找個元素加 `sticky top-0` 就能生效,遇到「明明照教科書寫法卻完全沒反應」時,先懷疑是不是這個坑,用 Playwright 量 `document.scrollingElement` 跟目標元素祖先鏈的 `overflow` 計算值來確認,不要憑感覺猜。**
  - **展開面板一開始直接把 padding(`py-6`)加在會被 GSAP 動畫 `height` 的那一層(`#continuousNavList`)上,結果收合時 inline style 明明是 `height: 0px`,`getComputedStyle` 量出來卻還是 `~49px`(剛好等於 `py-6` 上下加起來的高度),選單關閉後底下的內容(例如「OVERVIEW」)會有一小截若隱若現地穿幫。** 原因是 Tailwind 的全域 reset 把每個元素預設成 `box-sizing: border-box`,這個盒模型下 `height` 包含 padding 在內——把 `height` 硬壓成 `0` 時,padding 沒辦法一起被壓縮成負值,瀏覽器只能讓盒子維持在「至少跟 padding 一樣高」,inline 的 `height:0` 形同虛設。**修法是把視覺樣式(padding、背景、邊框、陰影)搬到裡面再包一層 div,外層(真正被 GSAP 動畫 height 的那個元素)只負責 `absolute` 定位 + `overflow:hidden` + 動畫的 `height`,不帶任何 padding**——兩層職責分開之後,外層才能真正收到 0。**這是一個容易再犯的坑:任何「用 GSAP/CSS 動畫 `height` 在 `0` 和內容高度之間切換」的元件,都不能讓真正被動畫的那一層自己帶 padding,padding 一定要下推到不參與動畫的內層,不管是不是這個 hamburger 選單,以後遇到類似的展開/收合需求都要先檢查這一點。**
  - **展開面板的連結一開始跟上面的 hamburger 按鈕沒對齊,連結整排貼在螢幕最左邊、比按鈕文字更靠左**——因為絕對定位元素(`#continuousNavList`)的 `left`/`right` 是相對「離自己最近的定位祖先的 padding box」,不是相對那個祖先自己內容實際起始的位置;`<nav>` 本身有 `px-8`,但這個 `px-8` 只影響 `nav` 自己「一般文件流」的子元素(像按鈕)往內縮,不會讓 `inset-x-0` 的絕對定位子元素自動跟著內縮——`inset-x-0` 對齊的是 `nav` 的 padding box 邊緣,等於整個螢幕的左右邊緣,而不是按鈕文字實際開始的 x 座標。**修法是在裡面那層(已經因為上一條坑而拆出來的、帶 padding 的 div)加上跟 `nav` 一樣的 `px-8`**,讓連結的文字內縮量跟按鈕一致,兩者才會左右對齊。這是延續上一條「視覺樣式都放在內層」的做法,只是這次是水平方向的內縮,不是垂直方向的 padding——同一個內層 div 現在同時處理水平對齊(`px-8`)和收合動畫需要的形狀(不影響 `height`,因為 `padding-left`/`padding-right` 在 `box-sizing:border-box` 下完全不影響 `height` 的計算,只有上下 padding 才會撐出上一條踩到的坑)。
  - **選單原本設計成點連結後自動收合,2026-09-03 同一天 Tim 又推翻這個設計,改成「持續展開著直到被使用者關閉」**——這件事再次提醒:一個功能做完、邏輯上合理,不代表最終會被採用,設計來回調整是常態,不要因為已經寫好一個行為就預設它是對的。
  - **2026-09-03 稍晚,Tim 要求平板/手機版「左邊 ← BACK、右邊 hamburger」共用同一排,不是像原本那樣分開顯示(← BACK 留在第一欄自己的 `.col-header` 裡、hamburger 另外自成一排)。** 做法是在 `<nav>` 的 fixed 頂欄裡「重新渲染一份」← BACK 連結(不是把原本那份從第一欄搬過來,原本那份留給桌面版用),兩份用 `lg:hidden`(fixed 頂欄這份,`<lg:` 才顯示)/`!hidden lg:!flex`(第一欄 `.col-header` 那份,`<lg:` 隱藏、`lg:` 才顯示)互斥顯示,不會同時出現兩個「← BACK」。**`.col-header` 這份要用 `!hidden`(強制 `!important`)而不是普通的 `hidden`,原因是 `.col-header` 這個 class 本身在 `css/style.css` 裡定義了 `display:flex`(全站共用的 96px 高標題列規則,見上面「元件慣例」),而且 `css/style.css` 這個 `<link>` 在 `case-study.html` 裡排在 Tailwind CDN 的 `<script>` 後面載入——兩條規則是同一個 specificity(都是單一 class 選擇器),CSS 疊層規則下「後載入的規則贏」,所以 `.col-header` 自己的 `display:flex` 會贏過普通的 `hidden`(`display:none`),平板/手機版還是會看到一個空的 96px 高橫條。加 `!` 讓 Tailwind 產生 `display:none !important`,保證不管載入順序或 specificity 高低都蓋得過去。**遇到「明明加了 `hidden` 卻沒有真的隱藏」的情況,先檢查目標元素身上有沒有其他自訂 class(不是 Tailwind 工具類)在 `css/style.css` 裡也定義了 `display`,兩者 specificity 相同時、載入順序會決定誰贏,不能只看有沒有加 `hidden` 這個 class 名稱本身。**
- **三欄的視覺對齊**:只有第一欄(作品基本信息)有分隔線(`lg:border-r`),第二欄(章節導覽)跟第三欄的每個 section 都刻意不加任何邊框——**2026-09-03 拿掉了原本每個 section 底部的 `border-b`**(原本的用意是分隔各個 section,但視覺上疊在一起太多線,Tim 要求拿掉,section 之間單純靠留白區隔)。第二欄跟第三欄的第一個區塊(Overview)桌面寬度的頂部內距都是實測調出來的 `lg:pt-36`,讓「Overview」這幾個字(不管是第二欄的連結、還是第三欄 Overview 區塊自己的 `<h2>`)的頂部都對齊第一欄大標題(`<h1>`)的頂部——用 Playwright 量過 `getBoundingClientRect().top`,三者(`<h1>`、第二欄 Overview 連結、第三欄 Overview 的 `<h2>`)完全相等(桌面寬度 1440px 下量測)。**只有 Overview 這個區塊需要 `lg:pt-36`,其餘 section 維持 `lg:pt-8`(桌面版上下內距 2026-09-06 從 `pt-20`/`pb-20` 一起縮小成 `pt-8`/`pb-8`,即 2rem,Tim 覺得原本 5rem 的段落間留白太大)**——因為只有 Overview 一開始(捲動位置在頂端時)會跟第一、二欄同一水平線,其餘 section 的位置是由上面內容的自然高度往下推算,不是固定在容器頂端,套用同一個 `pt-36` 沒有意義,也會把其餘 section 的內距拉得不必要地大。**這個 `pt-36` 數字是針對 `<h1>` 目前的字級/margin 調出來的,不是算出來的公式**——`<h1>` 的字級是響應式的(`text-[1.625rem] lg:text-[clamp(1.375rem,1.7vw,2.75rem)]`,細節見下面 `displayTitle` 那條),不同斷點/視窗寬度下 `<h1>` 實際佔的高度會有些微差異,`pt-36` 只保證在量測當下的寬度精確對齊,其他寬度可能有幾 px 的落差,如果之後要更精確,一樣要重新用 Playwright 量測調整,不要憑感覺改數字。拿掉 section 之間的 `border-b`、加大 Overview 的頂部內距都不影響可以捲到的內容範圍——第三欄還是同一個 `overflow-y-auto` 容器,捲到底一樣能看到最後一個 section 的最後一段文字,已經用 Playwright 把 `<main>` 捲到 `scrollHeight` 確認過,不會有任何內容被擠出捲動範圍外變得看不到。
...
- **大標題支援可選的 `displayTitle` 欄位**(HTML 字串,可以帶 `<br>` 手動換行),優先於 `title` 顯示在左欄的 `<h1>`——`document.title`(瀏覽器分頁標題)固定用 `title`,不受 `displayTitle` 影響。這個欄位兩種版面都支援(不是只有連續版面才有),沒有給的作品直接 fallback 用 `title`,不影響其餘作品。
  - **`<h1>` 的字級是 `text-[1.625rem] lg:text-[clamp(1.375rem,1.7vw,2.75rem)]`,不是固定斷點跳階,是刻意改成 `clamp()` 流動字級的結果。** 問題最早是 Tim 在窄手機螢幕(320px 寬)上發現 `displayTitle` 手動換行的第一行(`VisionControl.`)太長、超出 `intro-col` 可用寬度,`break-words` 被迫在單字中間硬斷(「VisionContr」/「ol.」),不是在 `<br>` 那個預期的斷點斷行。第一輪只調小了手機基準字級(`1.875rem`→`1.625rem`),當下用 Playwright 測過手機寬度沒問題就以為修好了——**結果這只修到手機這一段,桌面斷點(原本 `lg:text-[1.75rem] xl:text-[2.25rem] 2xl:text-[2.75rem]`)完全沒有測過,而且其實整段都在犯同一個 bug:用 Playwright 對 1024px 到 2560px 之間密集掃過一輪才發現,幾乎所有桌面寬度下「VisionControl.」都在斷字,只是視窗夠寬(超過 2560px 附近)才會剛好不斷。** 根本原因是 `intro-col` 的寬度是 `20.8333%` 加一個 `lg:min-w-[260px]` 下限,在 lg/xl 斷點附近視窗雖然變寬,欄寬幾乎沒變(卡在 260px 下限附近),但字級卻依斷點跳階變大,兩者完全不成比例。**教訓:改完一個斷點的字級沒問題,不代表其他斷點也沒事——遇到「這個容器寬度是某個比例算出來的」這種情況,系統性地測過整個寬度區間(不是只測幾個斷點邊界)才安全,尤其容器寬度有 `min-width` 下限、卻讓字級照斷點跳大這種組合特別容易出包。** 修法是拿掉整組斷點跳階,改用單一 `clamp()` 讓字級跟著視窗寬度連續縮放(不是每個斷點各自一個定值),`1.7vw` 這個係數是用 Playwright 在 1024px 到 2560px 之間逐一測出每個寬度下「VisionControl.」能維持一行的最大字級反推出來的,不是憑感覺選的數字,`clamp()` 的上下限(`1.375rem`/`2.75rem`)則保證不會在極端寬度下跑出安全範圍。之後如果又要調這個字級,一樣要重新用 Playwright 密集掃過寬度區間再改,不要只測幾個斷點邊界就收工。
...
**2026-09-03 統一定案,起因是首頁的圓角看起來「不像同一套系統」——同樣是圓角,小尺寸的膠囊/圓鈕看起來很圓,大尺寸的卡片幾乎看不出有圓角,兩者擺在一起像兩套不相關的風格。** 核心觀念:**同一個 `border-radius` 數值,在不同大小的容器上視覺上完全不是同一回事**——半徑相對容器尺寸的比例才是眼睛感知「圓不圓」的依據,不是那個像素數字本身。所以「統一圓角」不能只是把全站的 `rounded-*` 改成同一個 class,而是要分尺寸量級各自選一個視覺上「讀起來同一家族」的值:

- **小尺寸元件(高度大約在 26–70px 之間的膠囊狀/圓形元件)**:一律 `rounded-full`——首頁浮動 nav 列(`index.html` 的 `<nav>` 內層 `<div>`)、社群連結按鈕(LinkedIn/Instagram/Resume/GitHub)、Resume 手風琴收合圓鈕(`.toggle-icon`)、作品卡片的標籤膠囊(`buildWorkCard()`)全部屬於這個量級,套 `rounded-full` 之後實際算出來的半徑落在 13–18px 左右(高度的一半),彼此天生就是同一個視覺量級,不需要手動對齊數字。**2026-09-03 之前浮動 nav 列完全沒有圓角(直角),是這次抓出來的明顯落差**,補上 `rounded-full` 後才跟其他膠囊狀元件一致。
- **大尺寸容器(作品卡片縮圖這種動輒 400–700px 寬的容器)**:不能套 `rounded-full`(會整個變成橢圓形,不是想要的效果),但也不能沿用一個對小尺寸物件而言合理、對大容器而言小到看不出來的固定值——**原本是 `rounded-xl`(12px),在 600px+ 寬的卡片上幾乎沒有視覺存在感,跟旁邊 13–18px 量級的膠囊放在一起明顯「更方」。改成 `rounded-2xl`(16px)**,數字上跟小尺寸量級的實際半徑(13–18px)同一個級別,讓大小元件的轉角視覺上讀起來像同一套語言。**這不是精確算出來的比例公式,是刻意讓兩個量級的「絕對圓角半徑」數字彼此接近,不追求「半徑/容器寬度」這個比例本身相等**(那樣算出來的大容器圓角會誇張到不合理)。
- **之後新增任何首頁元件,先判斷屬於哪個尺寸量級再套對應的圓角規則**,不要憑感覺選一個 Tailwind `rounded-*` 級別;如果新元件的尺寸介於兩者之間或明顯超出這兩個量級,重新用 Playwright 量現有元件的 `getComputedStyle().borderRadius` 抓實際基準,不要憑印象猜數字。
